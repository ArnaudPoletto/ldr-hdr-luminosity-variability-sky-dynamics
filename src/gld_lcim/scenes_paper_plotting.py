...
def create_violin_plots(statistics: dict):
    ...
    overcast_stats = {k: statistics[k] for k in sorted(overcast_keys, key=lambda x: get_scene_number(x, "P2"))}
    clear_stats = {k: statistics[k] for k in sorted(clear_keys, key=lambda x: get_scene_number(x, "P2"))}
    ...
    fig.suptitle("P2 Distribution Comparison: Overcast vs Clear Conditions", fontsize=16, y=0.99)
    ...
    for idx, (element, label, color) in enumerate(zip(elements, labels, colors)):
        ...
        if element in ["diff_g_sums", "flow_sums"]:
            overcast_data = [x for x in overcast_data if x > (0.2 if element == "diff_g_sums" else 0.03)]
            clear_data = [x for x in clear_data if x > (0.2 if element == "diff_g_sums" else 0.03)]
